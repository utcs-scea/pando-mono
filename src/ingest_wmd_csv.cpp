...
auto generateWMDParser(
    ...
    pando::GlobalPtr<pando::Vector<pando::Vector<galois::WMDEdge>>> localReadEdges,
  ...
  return [localReadEdges, localRename, localReadVertices, totVerts, tokens](char* line) {
    auto vfunc = [localReadVertices, totVerts](WMDVertex v) {
      *totVerts += 1;
      return fmap(*localReadVertices, pushBack, v);
    };
    auto efunc = [localReadEdges, localRename](WMDEdge e, agile::TYPES inverseEdgeType) {
      WMDEdge inverseE = e;
      inverseE.type = inverseEdgeType;
      std::swap(inverseE.src, inverseE.dst);
      std::swap(inverseE.srcType, inverseE.dstType);
      PANDO_CHECK_RETURN(insertLocalEdgesPerThread(*localRename, *localReadEdges, e));
      PANDO_CHECK_RETURN(insertLocalEdgesPerThread(*localRename, *localReadEdges, inverseE));
...
void galois::loadWMDFilePerThread(
  ...
  auto parser = generateWMDParser(tokens, localReadEdges.getLocal(), perThreadRename.getLocal(),
