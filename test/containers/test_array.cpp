...
TEST(Array, Swap) {
  ...
  galois::Array<std::uint64_t> array0;
  ...
  galois::Array<std::uint64_t> array1;
...
TEST(Array, DataAccess) {
  ...
  // create array
  galois::Array<std::uint64_t> array;
  EXPECT_EQ(array.initialize(size), galois::Status::Success);
  for (std::uint64_t i = 0; i < size; i++) {
    array[i] = i;
  }
  for (std::uint64_t i = 0; i < size; i++) {
    EXPECT_EQ(array[i], i);
    EXPECT_EQ(&array[i], array.get(i));
  }

  auto checker = +[](galois::Span<std::uint64_t> array) {
    for (std::uint64_t i = 0; i < size; i++) {
      EXPECT_EQ(array[i], i);
      EXPECT_EQ(&array[i], array.get(i));
    }
  ...
  auto result =
      pando::executeOnWait(pando::Place{pando::NodeIndex{0}, pando::anyPod, pando::anyCore},
                           checker, galois::Span<std::uint64_t>(array.data(), size));
...
TEST(Array, Equality) {
  ...
  galois::Array<std::uint64_t> arr0;
  galois::Array<std::uint64_t> arr1;
  ...
  auto func = +[](galois::Array<std::uint64_t> arr0, galois::Array<std::uint64_t> arr1) {
    galois::Array<std::uint64_t> arr2;
