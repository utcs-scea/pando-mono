...
#define MEMNICBASE_ELI_PARAMS MEMLINKBASE_ELI_PARAMS, \
    ...
    protected:
        ...
        virtual void processInitMemRtrEvent(InitMemRtrEvent* imre) {
            ...
            if (destIDs.find(imre->info.id) != destIDs.end()) {
                if ((imre->info.tile == info.tile && imre->info.pod == info.pod && imre->info.pxn == info.pxn) || imre->info.shared)
                addDest(imre->info);
    ...
    private:
        ...
        void build(Params& params) {
            ...
            info.tile = params.find<uint32_t>("tile", 0);
            info.pod = params.find<uint32_t>("pod", 0);
            info.pxn = params.find<uint32_t>("pxn", 0);
            info.shared = params.find<bool>("shared", 0);
