...
// 1) Added AES encryption/decryption and ACM mechanism knobs to MEMCONTROLLER_ELI_PARAMS
...
/*
   ...
   memoryController.h:
   Declare class statics that allow other components to lookup the memory controller owning an address range.
   ...
   Makes member functions public:
   - translateToLocal()
   - tanslateToGlobal()

   Makes member fields public:
   - backing_
*/
...
#define MEMCONTROLLER_ELI_PARAMS {"backend.mem_size",    "(string) Size of physical memory. NEW REQUIREMENT: must include units in 'B' (SI ok). Simple fix: add 'MiB' to old value.", NULL},\
            ...
            {"aes_enable_security", "(uint) 0: disable AES encryption/decryption for writes/reads, 1: enable AES mechanisms", "0"},\
            {"acm_check_latency", "(uint) latency for acm check", "1"},\
            {"acm_row_hit", "(uint) are acm checks row hits", "1"},\
            {"aes_decryption_latency", "(uint) AES decryption latency (memory controller cycles)", "1"}
