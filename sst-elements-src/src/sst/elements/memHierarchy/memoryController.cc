...
// 1) Added parameter fields for to enable data encryption and access control mechanisms
// 2) Added param fields to enable security mechanism modeling implemented in memBackendConverter
...
/*
   ...
   Define class statics that allow other components to lookup the memory controller owning an address range.
   - AddrRangeToMC - std::vector<std::tuple<Addr, Addr, MemController*>>
   - AddrRangetoMCMutex - std::mutex
   - AddrRangetoMCSorted : std::atomic<bool>
   MemController instances register themselves atomically into the table on construction.
   Exactly one will sort the whole table during setup().
*/
/*
  <mrutt@cs.washington.edu> Thu Jul 27 15:20:34 2023 -0700:
  memoryController.cc
  Passes translateToLocal function to CustomCmdHandler constructor.
*/
...
MemController::MemController(ComponentId_t id, Params &params) : Component(id), backing_(NULL) {
    ...
    fixupParams( params, "aes_enable_security", "backendConverter.aes_enable_security");
    fixupParams( params, "aes_decryption_latency", "backendConverter.aes_decryption_latency");
    fixupParams( params, "acm_check_latency", "backendConverter.acm_check_latency");
    fixupParams( params, "acm_row_hit", "backendConverter.acm_row_hit");
    ...
    customCommandHandler_ = loadUserSubComponent<CustomCmdMemHandler>("customCmdHandler", ComponentInfo::SHARE_NONE,
            ...
            std::bind(static_cast<bool(MemController::*)(Addr,size_t,std::vector<uint8_t>&,bool,MemEventBase*)>(&MemController::monitorData), this, _1, _2, _3, _4, _5),
        ...
        case Command::FlushLineInv:
            {
                ...
                if ( ev->getPayloadSize() != 0 ) {
                    // put = new MemEvent(getName(), ev->getBaseAddr(), ev->getBaseAddr(), Command::PutM, ev->getPayload()/*Adithya modified further*/, 0);
                    // Alex
                    put = new MemEvent(getName(), ev->getBaseAddr(), ev->getBaseAddr(), Command::PutM, ev->getPayload());
                    put->setUserID(ev->getUserID());
...
bool MemController::monitorData(Addr addr, size_t bytes, std::vector<uint8_t> &data, bool equal, MemEventBase* ev) {
    bool monitor_hit;
    if (equal) {
        monitor_hit = true;
        for (size_t i = 0; i < bytes; i++)  {
    ...
    if (!monitor_hit) {
        ...
        monitorInfo monitor_info = {bytes, data, equal, *cme};
