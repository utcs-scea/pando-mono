...
// 1) Only users that brought in cachelines can evict cachelines, indicated with the userID that is stored as part of the cacheline 
// 2) Updated cacheline lookup mechanisms to include userID as criteria for cacheline search
// 3) New HandleEviction fucntion created to include userID as parameter and selection criteria for eviction
...
bool MESIInclusive::handleGetS(MemEvent * event, bool inMSHR) {
    ...
    uint64_t userID_ = event->getUserID();
    SharedCacheLine * line = cacheArray_->lookup(addr, true, userID_);
    // bool userID_matched = cacheArray_ -> lookup_userID(addr, userID_);
    ...
    switch (state) {
        case I:
            ...
            if (status == MemEventStatus::OK) { // Both MSHR insert and cache line allocation succeeded and there's no MSHR conflict
                line = cacheArray_->lookup(addr, false, userID_);
...
bool MESIInclusive::handleGetSResp(MemEvent * event, bool inMSHR) {
    ...
    SharedCacheLine * line = cacheArray_->lookup(addr, false);
    State state = line ? line->getState() : I;
    // std::cout << "handleGetsResp" << event->getResponseToID().first << std::endl;
    if (is_debug_event(event))
...
SharedCacheLine * MESIInclusive::allocateLine(MemEvent * event, SharedCacheLine * line) {
    ...
    // bool evicted = handleEviction(event->getBaseAddr(), line, event->getUserID());
    if (evicted) {
        notifyListenerOfEvict(line->getAddr(), lineSize_, event->getInstructionPointer());
        cacheArray_->replace(event->getBaseAddr(), line, event->getUserID());
        if (is_debug_event(event))
...
bool MESIInclusive::handleEviction(Addr addr, SharedCacheLine *& line, uint64_t userID) {
    if (!line)
        line = cacheArray_->findReplacementCandidate(addr, userID);
    State state = line->getState();
