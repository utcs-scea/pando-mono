...
// 1) Passing acmPassCheck variable to event to be set in MSHR to correctly handle potential ACM violation when memory miss is handled
...
bool Cache::processEvent(MemEventBase* ev, bool inMSHR) {
    ...
    event->acmPassCheck_ = ev->acmPassCheck_;
