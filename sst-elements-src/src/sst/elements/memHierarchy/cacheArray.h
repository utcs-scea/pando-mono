...
// 1) Included userID in cache tag to include in lookup to differentiate cachelines brought in by different users
// 2) Cache lookup and replacement includes userID as paarameters for search
// 3) Created new lookup function template that accepts userID as a parameter
...
template <class T>
T* CacheArray<T>::lookup(const Addr addr, bool updateReplacement, uint64_t userID) {
    Addr laddr = toLineAddr(addr);
    uint64_t userID_ = userID;
    int set = hash_->hash(0, laddr) % numSets_;
    int setBegin = set * associativity_;
    int setEnd = setBegin + associativity_;

    for (int i = setBegin; i < setEnd; i++) {
      //  printf("cacheArray: Existing addr = ");
        if (lines_[i]->getAddr() == addr) {
            // printf("lookup 1: Existing userid = %i New userid = %i\n", lines_[i]->getUserID(), userID_);
            // printf("Current state: %d\n", lines_[i]->getState());
            // if(lines_[i]->getState()==I){
            //     if (updateReplacement)
            //         replacementMgr_->update(i, lines_[i]->getReplacementInfo());
            //     return lines_[i];
            // }
            /*else*/
            if(lines_[i]->getUserID()==userID_){
                // printf("lookup 2: Existing userid = %i New userid = %i\n", lines_[i]->getUserID(), userID_);
                if (updateReplacement)
                    replacementMgr_->update(i, lines_[i]->getReplacementInfo());
                return lines_[i];
            }
...
/// Not used ////////////
template <class T>
T * CacheArray<T>::findReplacementCandidate(Addr addr, uint64_t userID) {
    Addr laddr = toLineAddr(addr);
    int set = hash_->hash(0, laddr) % numSets_;
/////////////
    int setBegin = set * associativity_;
    int setEnd = setBegin + associativity_;
    // printf("set = %d, setbegin =%d, setend = %d\n", set, setBegin, setEnd);
    int i;
    bool found= false;
    for (i = setBegin; i < setEnd; i++) {
        if (lines_[i]->getAddr() == addr) {
            // if (updateReplacement)
            //     replacementMgr_->update(i, lines_[i]->getReplacementInfo());
            // return lines_[i];
...
template <class T>
void CacheArray<T>::replace(Addr addr, T* candidate, uint64_t userID) {
    unsigned int index = candidate->getIndex();
    replacementMgr_->replaced(index);
    candidate->reset();
    candidate->setAddr(addr);
    // printf("Replace: Before updating userid = %i ", candidate->getUserID());
    candidate->setUserID(userID);
    // printf("After updating userid = %i\n", candidate->getUserID());
    replacementMgr_->update(index, lines_[index]->getReplacementInfo());
}
