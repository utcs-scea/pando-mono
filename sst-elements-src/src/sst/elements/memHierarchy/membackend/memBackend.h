...
// 1) Added m_aes_enable_security and m_aes_decryption_latency for data confidentiality performance modeling
...
#define MEMBACKEND_ELI_PARAMS {"debug_level",     "(uint) Debugging level: 0 (no output) to 10 (all output). Output also requires that SST Core be compiled with '--enable-debug'", "0"},\
    ...
    uint32_t        m_aes_enable_security;
    uint32_t        m_aes_decryption_latency;
