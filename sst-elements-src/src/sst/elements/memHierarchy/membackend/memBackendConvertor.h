...
// 1) Added several member variables to the MemBackendConverter class to enable memory encryption/decryption modeling
// 2) Functions isACMDependencyResolved() and isDecrypDependencyResolved() added to check dependency resolution and latency against m_cycleCount
...
#define MEMBACKENDCONVERTOR_ELI_STATS { "cycles_with_issue",                  "Total cycles with successful issue to back end",   "cycles",   1 },\
            ...
            { "requests_sent",                      "Total number of requests sent to membackend",      "requests", 1 },\
            { "acm_requests_sent",                  "Total number of acm requests sent to membackend",  "requests", 1 },\
    ...
    public:
        ...
        BaseReq( uint32_t reqId, ReqType(type) ) : m_reqId(reqId),
                 m_type(type), m_aesDoneCycle(0), m_fetchedFromMem(false),
                 m_acmDoneCycle(0), isACMReq(false), m_waitOnACM(0) { }
        virtual ~BaseReq() { }
        ...
        uint64_t m_aesDoneCycle;        // indicates cycle in which AES decryption will be complete (set for reads only)
        uint32_t m_reqId;               // moved from protected to public
        uint32_t m_flags;
        bool     m_fetchedFromMem;

        uint32_t m_userID;
        bool m_acmPassCheck;

        uint64_t getAesDoneCycle()                { return this->m_aesDoneCycle;  }
        void     setAesDoneCycle(uint64_t cycle)  { m_aesDoneCycle = cycle;       }

        uint32_t getUserID()                      { return this->m_userID; }
        void     setUserID(uint32_t userId)       { m_userID = userId;     }

        bool     getAcmPassCheck()                { return this->m_acmPassCheck;  }
        void     setAcmPassCheck(bool acmPass)    { m_acmPassCheck = acmPass;     }

        uint64_t m_acmDoneCycle;
        bool isACMReq;
        uint32_t m_waitOnACM; //indicate how many ACM req to wait on
  ...
  public:
    ...
    void processRequestEvent(SST::Event *event);

    // Member variables to hold knob values
    uint32_t m_secEnabled;
    uint32_t m_aesDecryptionLat;
    uint32_t m_aesEncryptOrDecrypt;
    uint32_t m_acmCheckComputationLat;
    bool m_acmRowHit;
    std::unordered_map<BaseReq*, BaseReq*> m_acmDependentReqs;

    // Dependency perventing from issuing response
    bool isDecrypDependencyResolved(const BaseReq *req) {
        if (!m_secEnabled) { return true;}
        if ((m_aesEncryptOrDecrypt == 2 || m_aesEncryptOrDecrypt == 3)) {
            return ((req->m_aesDoneCycle <= m_cycleCount) && req->m_fetchedFromMem);
        } else {
            return (req->m_fetchedFromMem);
        }
    }

    bool isACMDependencyResolved(const BaseReq *req) {
        if (!m_secEnabled) { return true;}
        return ((req->m_acmDoneCycle <= m_cycleCount) &&
                (!req->m_waitOnACM) && (!req->isACMReq) &&
                (req->m_fetchedFromMem));
    }
    ...
    uint32_t shouldIssueACMReq(MemReq * req) {
        if (!m_secEnabled) return 0;
        if ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::FlushLine) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::FlushLineInv) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::FlushAll)) {
            return 2;
        } else if ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::GetS) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::GetSX) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::Write) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutS) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutM) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutX) ||
                (static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutE)) {
            //testing with 2
            return 1;
        } else {
            // TODO: are there any reqs for which we don't generate ACM reqs ?
            return 0;
        }
        return 0;
    }
  ...
  private:
    ...
    bool setupMemReq( MemEvent* ev ) {
        ...
        // Alex
        if (DEBUG_LEVEL > 0) {
            std::cout << m_cycleCount << ": memBackendConverter: setupMemReq for new req " << id << ", event id " << ev->getID().first << std::endl;
            std::cout << "\t GetS: " << ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::GetS) ? "TRUE,":"FALSE,") \
                        ...
                        << "\t PutS: " << ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutS) ? "TRUE,":"FALSE,") \
                        << "\t PutM: " << ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutM) ? "TRUE,":"FALSE,") \
                        << "\t PutE: " << ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutE) ? "TRUE,":"FALSE,") \
                        << "\t PutX: " << ((static_cast<MemReq*>(req)->getMemEvent()->getCmd() == Command::PutX) ? "TRUE,":"FALSE,") \
        ...
        // Alex: set userID in memory request
        // TODO: enclose in knob
        req->setUserID(ev->getUserID());
        req->setAcmPassCheck(ev->getAcmPassCheck());
        // std::cout << "Calling setupMemReq() for req " << id << ", userID: " << req->getUserID() << std::endl;
        if (m_secEnabled) {
            req->m_aesDoneCycle = m_cycleCount;
            bool isWrite = req->isWrite() || (req->getMemEvent()->getCmd() == Command::FlushLine) || (req->getMemEvent()->getCmd() == Command::FlushLineInv) || (req->getMemEvent()->getCmd() == Command::FlushAll) || (req->getMemEvent()->getCmd() == Command::PutM);
            if (isWrite && (m_aesEncryptOrDecrypt == 1 || m_aesEncryptOrDecrypt == 3)) {
                req->m_aesDoneCycle = m_cycleCount + m_aesDecryptionLat;
            }

            // if ACM check required
            uint32_t numACMReqs = shouldIssueACMReq(req);
            uint32_t acmReq_id;
            ...
            if (numACMReqs > 0) {
                ...
                MemEvent *acmMemEvnt = new MemEvent(*ev);
                acmMemEvnt->setCmd(Command::GetS);

                //acm results in row miss
                if (!m_acmRowHit) {
                    Addr origAddr = acmMemEvnt->getBaseAddr();
                    if (DEBUG_LEVEL > 0) {
                        fprintf(stderr, "Address changed from %lx to %lx\n", origAddr,(origAddr ^ 0xFFFFFFFF));
                    }
                ...
                req->m_acmDoneCycle = m_cycleCount;
                req->m_waitOnACM = numACMReqs;
                m_acmDependentReqs[acmReq] = req;
                if (DEBUG_LEVEL > 0) {
                    fprintf(stderr, "pushing ACM LD Req: %p id: %u ev ID: %u for actual req: %p ev ID: %u @ %lu numACMReq: %u\n",
                            acmReq, acmReq_id, acmMemEvnt->getID().first, req, ev->getID().first, m_cycleCount, numACMReqs);
                }
            }

            if (DEBUG_LEVEL > 0) { std::cout << "\tm_aesDoneCycle for " << id << " set to " << req->m_aesDoneCycle << std::endl; }
        }
    ...
    Statistic<uint64_t>* stat_ReqSent;
    Statistic<uint64_t>* stat_ACMReqSent;
    ...
    /*
    Statistic<uint64_t>* stat_aes_readReqsDecrypted;
    Statistic<uint64_t>* stat_aes_writeReqsEncrypted;
    Statistic<uint64_t>* stat_aes_readDecryptionStalls;
    Statistic<uint64_t>* stat_aes_writeEncryptionStalls;
    Statistic<uint64_t>* stat_aes_readDecryptionStallCycles;
    Statistic<uint64_t>* stat_aes_writeEncryptionStallCycles;
    */
