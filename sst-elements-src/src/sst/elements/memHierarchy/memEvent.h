...
// 1) Updated initialization of MemEvents to include default userID and acmPassCheck 
...
public:
    ...
    MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd, uint32_t size) : MemEventBase(src, cmd) {
        ...
        baseAddr_ = baseAddr;
        size_ = size;
        userID_ = 0;            // changing function prototype required redefinition of all MemEvent instantiations, only set userID after instatiation where needed with setUserID
        acmPassCheck_ = true;   // set acmPassCheck_ default to true
    }
    MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd, std::vector<uint8_t>& data) : MemEventBase(src, cmd) {
        ...
        baseAddr_ = baseAddr;
        userID_ = 0;            // changing function prototype required redefinition of all MemEvent instantiations, only set userID after instatiation where needed with setUserID
        acmPassCheck_ = true;   // set acmPassCheck default to true
        setPayload(data);
    }
    ...
    MemEvent* makeResponse() override {
        ...
        me->vAddr_        = vAddr_;
        me->userID_       = userID_;
        me->acmPassCheck_ = acmPassCheck_;
        return me;
    }
    ...
    uint64_t getUserID(void) const { return userID_; }
