...
// 1) Modified Event->Request and Request->Event converters to include the userID and acmPassCheck flags copy as part of conversion
...
/*
  <mrutt@cs.washington.edu> Tue Jul 25 13:09:44 2023 -0700
  standardInterface.cc:
  Correctly sets CustomReq routing data and thread id in StandardInterface::MemEventConverter::convert(CustomReq *).
*/
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::Read* req) {
    ...
    // Set userId and acmPassCheck from req
    read->setUserID(req->userID);
    read->setAcmPassCheck(req->acmPassCheck);
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::Write* req) {
    ...
    // Set userID and acmPassCheck from request
    write->setUserID(req->userID);
    write->setAcmPassCheck(req->acmPassCheck);
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::LoadLink* req) {
    ...
    // MemEvent* load = new MemEvent(iface->getName(), req->pAddr, bAddr, Command::GetS, req->size, 0);
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::StoreConditional* req) {
    Addr bAddr = (iface->lineSize_ == 0 || req->getNoncacheable()) ? req->pAddr : req->pAddr & iface->baseAddrMask_;
    // TODO: update with userId
    // MemEvent* store = new MemEvent(iface->getName(), req->pAddr, bAddr, Command::Write, req->data, 0);
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::ReadResp* resp) { 
    ...
    meresp->setAcmPassCheck(mereq->getAcmPassCheck());
    // std::cout << "convert(readresp->event) reqId " << mereq->getID().first << ", respId " << meresp->getID().first << ", acmPassCheck(req): " << mereq->getAcmPassCheck() << ", acmPAssCheck(resp) " << meresp->getAcmPassCheck() << std::endl;
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::WriteResp* resp) {
    ...
    MemEvent* meresp = mereq->makeResponse();
    meresp->setAcmPassCheck(mereq->getAcmPassCheck());
    //std::cout << "convert(writeresp->event) reqId " << mereq->getID().first << ", respId " << meresp->getID().first << ", acmPassCheck(req): " << mereq->getAcmPassCheck() << ", acmPAssCheck(resp) " << meresp->getAcmPassCheck() << std::endl;
    if (!resp->getSuccess()) {
        meresp->setFail();
    }
    return meresp;
...
StandardMem::Request* StandardInterface::convertResponseGetSResp(StandardMem::Request* req, MemEventBase* meb) {
    ...
    // Set acmPassCheck from the memEvent
    resp->acmPassCheck = meb->getAcmPassCheck();
...
StandardMem::Request* StandardInterface::convertResponseWriteResp(StandardMem::Request* req, MemEventBase* meb) {
    ...
    // Set acmPassCheck from the memEvent
    resp->acmPassCheck = me->getAcmPassCheck();
    if (!me->success()) {
        resp->setFail();
    }
    return resp;
