...
// 1) Update read and write commit function prototypes to include userID as parameter
...
class ArielCore : public ComponentExtension {
    ...
    public:
        ...
        void unfence();
        void finishCore();
        void createReadEvent(uint64_t addr, uint32_t size, int userID);
        void createWriteEvent(uint64_t addr, uint32_t size, const uint8_t* payload, int userID);
        ...
        void commitReadEvent(const uint64_t address, const uint64_t virtAddr, const uint32_t length, const int userID=0);
        void commitWriteEvent(const uint64_t address, const uint64_t virtAddr, const uint32_t length, const uint8_t* payload, const int userID=0);
    ...
    private:
        bool processNextEvent(int userID);
        bool refillQueue(int userID);
        ...
        uint64_t userID;
        uint64_t num_memop;
        uint64_t memop_violation;
        bool aes_enable_security;
        Link* RtlLink;
