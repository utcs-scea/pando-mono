...
// 1) Check added to stop Ariel instance if simulated ACM violation has occurred. Halts if an ACM violation has been identified.
...
/** End memory manager subcomponent parameter translation */
    ...
    if (NULL != (memmgr = loadUserSubComponent<ArielMemoryManager>("memmgr"))) {
        ...
        output->verbose(CALL_INFO, 1, 0, "Loaded memmgr params\n");
        ...
        output->verbose(CALL_INFO, 1, 0, "Loaded memmgr\n");
...
bool ArielCPU::tick( SST::Cycle_t cycle) {
    ...
    // Gaurav - check to kill thread
    // TODO: modify to kill thread on ACM pass check failure
    bool flag = false;
    for(uint32_t i = 0; i < core_count; ++i) {
         if(cpu_cores[i]->temp_finish()) {
            flag=true;
            printf("\n=========Killing User %d====================\n",cpu_cores[0]->getuser());
        }
    }
