...
// 1) Added in userID field that is set by Python configuration file to determine userID of application running on core
// 2) acmPassCheck flag is created to see if simulated ACM violation has occurred. Sets temp_halt flag for Ariel CPU to stop execution for individual user.
// 2) UserID is included in read and write commits to populate userID field in Read and Write packets
...
ArielCore::ArielCore(ComponentId_t id, ArielTunnel *tunnel,
    ...
    aes_enable_security = params.find<int>("aes_enable_security", 0);
    output->verbose(CALL_INFO, 1, 0, "Ariel core with coreID %i aes_enable_security=%i\n", coreID, aes_enable_security);

    userID = params.find<int>("userID", 0);
    if(!aes_enable_security){
        userID=0;
    }
    output->verbose(CALL_INFO, 1, 0, "Ariel core with coreID %i registered with userID %i\n", coreID, userID);
    
    memop_violation = params.find<int>("memop_violation", 0);
    output->verbose(CALL_INFO, 1, 0, "Ariel num_memops coreID %i num_memops till violation: %i\n", coreID, memop_violation);
...
void ArielCore::commitReadEvent(const uint64_t address,
            const uint64_t virtAddress, const uint32_t length,const int userID) {
    if(length > 0) {
        StandardMem::Read *req = new StandardMem::Read(address, length, 0, virtAddress /*adithya*/, 0, 0, userID);

        // Tracking number of read memops until access control violation
        num_memop++;
        req->acmPassCheck = true;
        if(aes_enable_security){
            if (num_memop == memop_violation) {
                req->acmPassCheck = false;
            }
        }
#ifdef HAVE_CUDA
        if(isGpuEx()){
...
void ArielCore::commitWriteEvent(const uint64_t address,
        const uint64_t virtAddress, const uint32_t length, const uint8_t* payload, const int userID) {
    ...
    if(length > 0) {
        ...
        StandardMem::Write *req = new StandardMem::Write(address, length, data, false, 0, virtAddress, 0, 0, userID);
        // Tracking number of write memops until access control violation
        req->acmPassCheck = true;
        num_memop++;
        if(aes_enable_security){
            if (num_memop == memop_violation) { req->acmPassCheck = false; }
        }
...
void ArielCore::createReadEvent(uint64_t address, uint32_t length, int userID) {
    ArielReadEvent* ev = new ArielReadEvent(address, length, userID);
    coreQ->push(ev);
...
void ArielCore::createWriteEvent(uint64_t address, uint32_t length, const uint8_t* payload, int userID) {
    ArielWriteEvent* ev = new ArielWriteEvent(address, length, payload, userID);
    coreQ->push(ev);
...
bool ArielCore::refillQueue(int userID) {
        ...
        switch(ac.command) {
            ...
            case ARIEL_START_INSTRUCTION:
                ...
                while(ac.command != ARIEL_END_INSTRUCTION) {
                        ...
                        //output->verbose(CALL_INFO, 1, 0, "Inside refillqueue: Registering ArielCPU userID as %i\n", userID);

                        switch(ac.command) {
                            case ARIEL_PERFORM_READ:
                                    createReadEvent(ac.inst.addr, ac.inst.size, userID);
                                    break;

                            case ARIEL_PERFORM_WRITE:
                                    createWriteEvent(ac.inst.addr, ac.inst.size, &ac.inst.payload[0], userID);
                                    break;
...
void ArielCore::handleReadRequest(ArielReadEvent* rEv) {
    ...
    const uint64_t physAddr = (userID<<28)+(memmgr->translateAddress(readAddress));
    ...
    if((addr_offset + readLength) <= cacheLineSize) {
        ...
        const int userID = rEv->getuserID();
        commitReadEvent(physAddr, readAddress, (uint32_t) readLength, userID);
    } else {
        ...
        const uint64_t physRightAddr = (userID<<28)+memmgr->translateAddress(rightAddr);
        ...
        commitReadEvent(physLeftAddr, leftAddr, (uint32_t) leftSize, userID);
        commitReadEvent(physRightAddr, rightAddr, (uint32_t) rightSize, userID);
...
/*    if(writeLength > cacheLineSize) {
    ...
    const uint64_t physAddr = (userID<<28)+memmgr->translateAddress(writeAddress);
    const uint64_t addr_offset  = physAddr % ((uint64_t) cacheLineSize);
    ...
    if((addr_offset + writeLength) <= cacheLineSize) {
        ...
        if( writePayloads ) {
            ...
            commitWriteEvent(physAddr, writeAddress, (uint32_t) writeLength, payloadPtr, userID);
        } else {
            // commitWriteEvent(physAddr, writeAddress, (uint32_t) writeLength, NULL);
            commitWriteEvent(physAddr, writeAddress, (uint32_t) writeLength, NULL, userID);
        }
    } else {
        ...
        if( writePayloads ) {
            ...
            commitWriteEvent(physLeftAddr, leftAddr, (uint32_t) leftSize, payloadPtr, userID);
            commitWriteEvent(physRightAddr, rightAddr, (uint32_t) rightSize, &payloadPtr[leftSize], userID);
        } else {
            ...
            commitWriteEvent(physLeftAddr, leftAddr, (uint32_t) leftSize, NULL, userID);
            commitWriteEvent(physRightAddr, rightAddr, (uint32_t) rightSize, NULL, userID);
        }
...
void ArielCore::handleRtlAckEvent(SST::Event* e) {
    ...
    if(ev->RtlData.rtl_inp_ptr != nullptr) {
        ...
        ArielReadEvent *are = new ArielReadEvent((uint64_t)ev->RtlData.rtl_inp_ptr, (uint64_t)ev->RtlData.rtl_inp_size, 0);
...
bool ArielCore::processNextEvent(int userID) {
    ...
    if(coreQ->empty()) {
        ...
        bool addedItems = refillQueue(userID);
    ...
    if(!isHalted) {
        ...
        if(!isStalled) {
                for(uint32_t i = 0; i < maxIssuePerCycle; ++i) {
                    ...
                    bool didProcess = processNextEvent(userID);
