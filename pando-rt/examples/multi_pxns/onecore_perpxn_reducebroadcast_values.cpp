...
/** @file onecore_perpxn_reducebroadcast_values.cpp
 ...
 *  4) A single core for each PXN broadcasts the combined (reduced) owning values to other PXNs.
 */
 ...
 */
template <typename T>
void waitUntil(pando::GlobalPtr<T> dones, T expected, std::int16_t numNodes) {
...
void reduceOwnValues(pando::GlobalPtr<std::int64_t> srcArray,
  ...
  for (std::int32_t i = 0; i < numCoresPerNode; ++i) {
    if (i % placeDims.node.id == thisNodeId) {
      ownArray[i] = ownArray[i] + (thisNodeId * srcArray[i]);
    }
  }
...
void broadcastOwnValues(pando::GlobalPtr<std::int64_t> srcArray,
  ...
  for (std::int32_t i = 0; i < numCoresPerNode; ++i) {
    if (i % placeDims.node.id == thisNodeId) {
      ...
      srcArray[i] = static_cast<std::int64_t>(ownArray[i]);
    }
  }
  dones[thisNodeId] = true;
}
...
int pandoMain(int, char**) {
  ...
  if (thisPlace.node.id == 0) {
    ...
    broadcastValues(distArray, numCoresPerNode, boolDones);
