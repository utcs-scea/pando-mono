...
/** @file allcores_reduce_values.cpp
 ...
 */
void setValue(pando::GlobalPtr<std::int64_t> localArray, std::int64_t initialValue,
              std::int32_t index, pando::GlobalPtr<bool> coreDone) {
  localArray[index] = initialValue;
  *coreDone = true;
}
...
void reduceValue(pando::GlobalPtr<std::int64_t> ownArray, std::int64_t srcValue, std::int32_t index,
  ...
  ownArray[index] = ownArray[index] + srcValue;
  coreDone[index] = true;
}
...
void reduceValues(pando::GlobalPtr<pando::GlobalPtr<std::int64_t>>& distArray,
                  [[maybe_unused]] std::int32_t numCoresPerNode, pando::GlobalPtr<bool> boolDones,
                  [[maybe_unused]] pando::GlobalPtr<pando::GlobalPtr<bool>> distCoreDones) {
  const auto placeDims = pando::getPlaceDims();
