...
void load(GlobalAddress srcGlobalAddr, std::size_t n, void* dstNativePtr) {
  ...
  for (std::size_t i = 0; i < numBlocks; i++) {
    ...
    *(blockDst + i) = DrvAPI::read<BlockType>(blockSrc + bytesOffset, DrvAPI::program_phase);
  }
  ...
  for (std::size_t i = 0; i < remainderBytes; i++) {
    *(byteDst + i) = DrvAPI::read<std::byte>(byteSrc + i, DrvAPI::program_phase);
  }
...
void store(GlobalAddress dstGlobalAddr, std::size_t n, const void* srcNativePtr) {
  ...
  for (std::size_t i = 0; i < numBlocks; i++) {
    ...
    DrvAPI::write<BlockType>(blockDst + offset, DrvAPI::program_phase, blockData);
  }
  ...
  for (std::size_t i = 0; i < remainderBytes; i++) {
    ...
    DrvAPI::write<std::byte>(byteDst + i, DrvAPI::program_phase, byteData);
  }
