...
void Cores::initializeQueues() {
  ...
  // purpose.
  if (DrvAPI::atomic_cas(&coreState, DrvAPI::phase_t::PHASE_OTHER, +CoreState::Stopped, +CoreState::Idle) ==
      +CoreState::Stopped) {
    ...
    DrvAPI::atomic_add(toNativeDrvPointerOnDram(numCoresReady, NodeIndex(0)), DrvAPI::phase_t::PHASE_OTHER, 1u);
  }
...
void Cores::finalizeQueues() {
  ...
  // purpose.
  if (DrvAPI::atomic_cas(&coreState, DrvAPI::phase_t::PHASE_OTHER, +CoreState::Ready, +CoreState::Idle) == +CoreState::Ready) {
...
void Cores::finalize() {
  DrvAPI::atomic_add(toNativeDrvPointerOnDram(numPxnsDone, NodeIndex(0)), DrvAPI::phase_t::PHASE_OTHER, 1u);
}
