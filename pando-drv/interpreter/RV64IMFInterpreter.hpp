...
public:
  ...
  void visitFMADD_S_DYN(RISCVHart& hart, RISCVInstruction& i) override {
    ...
    hart.f(i.rd()) = std::fmaf(hart.sf(i.rs1()), hart.sf(i.rs2()), hart.sf(i.rs3()));
    hart.pc() += 4;
  }
  ...
  void visitFEQ_S(RISCVHart& hart, RISCVInstruction& i) override {
    ...
    hart.x(i.rd()) = std::isnan(f1) || std::isnan(f2) ? 0 : hart.sf(i.rs1()) == hart.sf(i.rs2());
    hart.pc() += 4;
  }
  ...
  void visitFCLASS_S(RISCVHart& hart, RISCVInstruction& i) override {
    ...
    uint64_t result = 0;
    riscvbits::setbit(result, RISCVHart::FCLASS_IS_NEG_INF, std::isinf(f) && std::signbit(f));
    riscvbits::setbit(result, RISCVHart::FCLASS_IS_POS_INF, std::isinf(f) && !std::signbit(f));
    ...
    riscvbits::setbit(result, RISCVHart::FCLASS_IS_POS_ZERO,
                      std::fpclassify(f) == FP_ZERO && !std::signbit(f));
    riscvbits::setbit(result, RISCVHart::FCLASS_IS_SIGNAL_NAN, false);
    riscvbits::setbit(result, RISCVHart::FCLASS_IS_QUIET_NAN, std::isnan(f));
    hart.x(i.rd()) = result;
    hart.pc() += 4;
  }
};
