...
#define pr_info(fmt, ...)                       \
    do { printf("[ICACHE BACKING]: " fmt, ##__VA_ARGS__); } while (0)
...
public:
    ...
    Elf64_Ehdr *ehdr() const { return reinterpret_cast<Elf64_Ehdr*>(data_); }
    ...
    uint32_t read(Elf64_Addr addr) {
        ...
        pr_debug("found text phdr for addr 0x%08x: p_vaddr = 0x%08x, p_paddr = 0x%08x, p_offset = 0x%08x\n"
                ,addr
                ,ph->p_vaddr
                ,ph->p_paddr
                ,ph->p_offset);
    ...
    void printSectionInfo(int idx) {
        ...
        ss << ", sh_flags: " << std::hex << std::setw(10) << std::setfill('0') << sh->sh_flags << std::setfill(' ');
