 ...
 */
CustomCmdMemHandler::MemEventInfo DrvCmdMemHandler::receive(MemEventBase* ev) {
  output.verbose(CALL_INFO, 1, 0, "%s\n", __PRETTY_FUNCTION__);
 ...
 */
Interfaces::StandardMem::CustomData* DrvCmdMemHandler::ready(MemEventBase* ev) {
  output.verbose(CALL_INFO, 1, 0, "%s\n", __PRETTY_FUNCTION__);
 ...
 * including the return flags. This function should return a response event or null if none needed.
 ...
 *  translateLocalT
 */
MemEventBase* DrvCmdMemHandler::finish(MemEventBase* ev, uint32_t flags) {
  output.verbose(CALL_INFO, 1, 0, "%s\n", __PRETTY_FUNCTION__);
  if (ev->queryFlag(MemEventBase::F_NORESPONSE) || ((flags & MemEventBase::F_NORESPONSE) > 0)) {
    // posted request
  ...
  if (!ard->extdata.empty()) {
    DrvAPI::atomic_modify(&ard->wdata[0], &ard->rdata[0], &ard->extdata[0], &ard->wdata[0],
                          ard->opcode, ard->getSize());
  } else {
    DrvAPI::atomic_modify(&ard->wdata[0], &ard->rdata[0], &ard->wdata[0], ard->opcode,
                          ard->getSize());
  }
  // write-back
 ...
 */
bool DrvSimpleMemBackend::issueCustomRequest(ReqId req_id,
