...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(RISCVCore, "Drv", "RISCVCore", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                             "RISCV Core", COMPONENT_CATEGORY_PROCESSOR)
  // DOCUMENT PARAMETERS
  SST_ELI_DOCUMENT_PARAMS(
      /* input */
      {"program", "Program to run", "/path/to/r64elf"}, {"load", "Load program into memory", "0"},
      {"release_reset", "Time to release from reset", "0"},
      /* control */
      {"mmio_addr_start", 0, "MMIO start address"}, {"mmio_addr_end", 0, "MMIO end address"},
      /* system config */
      DRV_SYS_CONFIG_PARAMETERS{"sp", "[Core Value. ...]", ""},
      {"clock", "Clock rate in Hz", "1GHz"}, {"num_harts", "Number of harts", "1"},
      {"core", "Core ID", "0"}, {"pod", "Pod ID", "0"}, {"pxn", "PXN ID", "0"},
      /* debugging */
      {"verbose", "Verbosity of output", "0"}, {"debug_memory", "Debug memory requests", "0"},
      {"debug_idle", "Debug idle cycles", "0"}, {"debug_requests", "Debug requests", "0"},
      {"debug_responses", "Debug responses", "0"}, {"debug_syscalls", "Debug system calls", "0"},
      {"debug_mmio", "Debug MMIO requests", "0"}, {"isa_test", "Report ISA tests results", "0"},
  ...
  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"memory", "Interface to a memory hierarchy",
...
#undef DEFINE_DRV_STAT
    auto parent =
        SST::ELI::InfoStats<std::conditional<(__EliDerivedLevel > __EliBaseLevel), __LocalEliBase,
                                             __ParentEliBase>::type>::get();
  ...
  static constexpr uint32_t DEBUG_MMIO =
      (1 << 28); //!< debug messages we expect to see when receiving mmio requests
   ...
   * get system info
   */
  DrvAPI::DrvAPISysConfig sys() const {
    return sys_config_.config();
  }

  /**
   * get the max write request size
   */
  size_t getMaxReqSize() const {
    return sys().numNWObufDwords() * sizeof(uint64_t);
  }
