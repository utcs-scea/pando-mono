...
void RISCVCore::configureMemory(Params& params) {
  ...
  mem_->setMemoryMappedAddressRegion(mmio_start_.encode(),
...
void RISCVCore::configureLinks(Params& params) {
  loopback_ = configureSelfLink("loopback",
                                new Event::Handler<RISCVCore>(this, &RISCVCore::handleLoopback));
...
RISCVCore::RISCVCore(ComponentId_t id, Params& params)
    : Component(id), mem_(nullptr), icache_(nullptr), harts_(), last_hart_(0) {
...
DrvAPI::DrvAPIPAddress RISCVCore::toPhysicalAddress(uint64_t addr) const {
  return DrvAPI::DrvAPIVAddress::to_physical(addr, pxn_, pod_, DrvAPI::coreYFromId(core_),
                                             DrvAPI::coreXFromId(core_));
}
...
void RISCVCore::handleResetWrite(uint64_t v) {
  output_.verbose(CALL_INFO, 0, DEBUG_MMIO,
                  "PXN %d: POD %d: CORE %d: Received reset write request\n", pxn_, pod_, core_);
  if (v == 0) {
    // deassert reset
    for (RISCVSimHart& hart : harts_) {
      hart.reset() = false;
...
void RISCVCore::handleMMIOWrite(Interfaces::StandardMem::Write* write_req) {
  ...
  output_.verbose(CALL_INFO, 0, DEBUG_MMIO,
                  "PXN %d: POD %d: CORE %d: Received MMIO write request\n", pxn_, pod_, core_);

  if (write_req->size != sizeof(uint64_t)) {
    output_.fatal(CALL_INFO, -1,
                  "PXN %d: POD %d: CORE %d: MMIO write request size is not 8 bytes\n", pxn_, pod_,
                  core_);
  }
  ...
  switch (paddr.ctrl_offset()) {
    ...
    default:
      output_.verbose(CALL_INFO, 0, DEBUG_MMIO,
                      "PXN %d: POD %d: CORE %d: Unhandled MMIO write request\n", pxn_, pod_, core_);
  }
  mem_->send(rsp);
  delete write_req;
}
...
bool RISCVCore::tick(Cycle_t cycle) {
  ...
  if (hart_id != NO_HART) {
    ...
    output_.verbose(CALL_INFO, 100, 0,
                    "Ticking hart %2d: pc = 0x%016" PRIx64 ", instr = 0x%08" PRIx32 " (%s)\n",
                    hart_id, pc, inst, i->getMnemonic());
