...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(DrvCore, "Drv", "DrvCore", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                             "Drv Core", COMPONENT_CATEGORY_UNCATEGORIZED)
  // Document the parameters that this component accepts
  SST_ELI_DOCUMENT_PARAMS(
      /* input */
      {"executable", "Path to user program", ""}, {"argv", "List of arguments for program", ""},
      /* system config */
      ...
      {"id", "ID for the core", "0"}, {"pod", "Pod ID of this core", "0"},
  ...
  SST_ELI_DOCUMENT_PORTS({"loopback", "A loopback link", {"Drv.DrvEvent", ""}}, )

  // Document the subcomponents that this component has
  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"memory", "Interface to memory hierarchy",
                                       "Drv::DrvMemory"}, )
  ...
  // DOCUMENT STATISTICS
  SST_ELI_DOCUMENT_STATISTICS({"load_l1sp", "Number of loads to local L1SP", "count", 1},
                              ...
                              {"tag_cycles", "number of cycles spent executing with a tag", "count",
                               1},
  ...
  static constexpr uint32_t DEBUG_CLK =
      (1 << 31); //!< debug messages we expect to see during clock ticks
  static constexpr uint32_t DEBUG_REQ =
      (1 << 30); //!< debug messages we expect to see when receiving requests
  static constexpr uint32_t DEBUG_RSP =
      (1 << 29); //!< debug messages we expect to see when receiving responses
  static constexpr uint32_t DEBUG_LOOPBACK =
      (1 << 28); //!< debug messages we expect to see when receiving loopback events
  static constexpr uint32_t DEBUG_MMIO =
      (1 << 27); //!< debug messages we expect to see when receiving mmio events
  ...
  static constexpr uint32_t TRACE_REMOTE_PXN_MEMORY =
      (TRACE_REMOTE_PXN_STORE | TRACE_REMOTE_PXN_LOAD |
       TRACE_REMOTE_PXN_ATOMIC); //!< trace remote memory events
private:
  std::unique_ptr<SST::Output> trace_; //!< for tracing

public:
  void traceRemotePxnMem(uint32_t trace_mask, const char* opname, DrvAPI::DrvAPIPAddress paddr,
                         DrvThread* thread) const {
    trace_->verbose(CALL_INFO, 0, trace_mask,
                    "OP=%s:SRC_PXN=%d:SRC_POD=%d:SRC_CORE=%d:SRC_THREAD=%d:DST_PXN=%d:ADDR=%s\n",
                    opname, pxn_, pod_, id_, getThreadID(thread), (int)paddr.pxn(),
...
private:
  ...
  drv_api_get_thread_context_t
      get_thread_context_; //!< the get_thread_context function in the executable
  drv_api_set_thread_context_t
      set_thread_context_;                  //!< the set_thread_context function in the executable
