...
public:
  ...
  struct ToNativeMetaData {
    ...
    struct InterleaveDecoder {
       ...
       */
      std::tuple<uint64_t, uint64_t> getBankOffset(uint64_t addr) const {
    ...
    std::vector<std::vector<std::vector<record_type>>>
        l1sp_mcs; //!< l1sps mem controllers and their address ranges
    std::vector<std::vector<std::vector<record_type>>>
        l2sp_mcs; //!< l2sps mem controllers and their address ranges
    std::vector<std::vector<record_type>>
        dram_mcs; //!< drams mem controllers and their address ranges
  ...
  SST_ELI_REGISTER_SUBCOMPONENT(SST::Drv::DrvStdMemory, "Drv", "DrvStdMemory",
                                SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                "Memory that interfaces with memHierarchy components",
                                SST::Drv::DrvMemory)

  // register subcomponent slots
  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"memory", "Memory component",
   ...
   * @param mem_req
   */
  void sendRequest(DrvCore* core, DrvThread* thread,
