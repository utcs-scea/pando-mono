...
void DrvCore::configureOutput(SST::Params& params) {
  ...
  std::stringstream ss;
  ss << "[DrvCore "
     << "{PXN=" << std::setw(2) << pxn_ << ",POD=" << std::setw(2) << pod_
     << ",CORE=" << std::setw(2) << id_ << "} @t: @f:@l: @p] ";
...
void DrvCore::configureMemory(SST::Params& params) {
  ...
  if (!memory_) {
    ...
    memory_ = loadAnonymousSubComponent<DrvMemory>("Drv.DrvSimpleMemory", "memory", 0,
                                                   ComponentInfo::SHARE_NONE, mem_params, this);
    if (!memory_) {
      output_->fatal(CALL_INFO, -1, "unable to load memory subcomponent\n");
    }
  }
  DrvAPI::DrvAPIAddress dram_base_default = DrvAPI::DrvAPIVAddress::MainMemBase(pxn_).encode();
  ...
  DrvAPI::DrvAPISection::GetSection(DrvAPI::DrvAPIMemoryL2SP).setBase(l2sp_base, pxn_, pod_, id_);
  ...
  DrvAPI::DrvAPISection::GetSection(DrvAPI::DrvAPIMemoryL1SP).setBase(l1sp_base, pxn_, pod_, id_);
}
...
void DrvCore::configureOtherLinks(SST::Params& params) {
  loopback_ =
      configureSelfLink("loopback", new Event::Handler<DrvCore>(this, &DrvCore::handleLoopback));
...
void DrvCore::configureSysConfig(SST::Params& params) {
  ...
  output_->verbose(CALL_INFO, 1, DEBUG_INIT,
                   ...
                   "core_threads = %d"
                   "\n",
                   cfg.numPXN(), cfg.numPXNPods(), cfg.numPodCores(), numThreads());
}
...
void DrvCore::handleThreadStateAfterYield(DrvThread* thread) {
  ...
  // handle termination
  std::shared_ptr<DrvAPI::DrvAPITerminate> term_req =
      std::dynamic_pointer_cast<DrvAPI::DrvAPITerminate>(state);
  if (term_req) {
