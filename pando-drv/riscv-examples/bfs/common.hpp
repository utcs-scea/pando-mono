...
static inline int threads() {
  return DrvAPI::numPodCores() * THREADS_PER_CORE;
}
static inline int my_thread() {
  return DrvAPI::myThreadId();
}
#else
static inline int threads() {
  return numPodCores() * myCoreThreads();
}
static inline int my_thread() {
  return myCoreThreads() * myCoreId() + myThreadId();
}
#endif
...
#ifdef COMMAND_PROCESSOR
static inline int core() {
  return DrvAPI::myCoreId();
}
static inline int thread_on_core() {
  return DrvAPI::myThreadId();
}
#else
static inline int core() {
...
#define pr_info(fmt, ...) \
  thread_safe_printf("PH: Core %d, Thread %d: " fmt, core(), thread_on_core(), ##__VA_ARGS__)

#define DEBUG
#ifdef DEBUG
#define pr_dbg(fmt, ...) pr_info(fmt, ##__VA_ARGS__)
#else
#define pr_dbg(fmt, ...)
#endif
 ...
 */
#ifdef COMMAND_PROCESSOR
#define THREADS (DrvAPI::myCoreThreads() * DrvAPI::numPodCores())
#else
...
//////////////////////////////
#ifdef COMMAND_PROCESSOR
#define REF_CLASS_BEGIN(type)               DRV_API_REF_CLASS_BEGIN(type)

#define REF_CLASS_DATA_MEMBER(type, member) DRV_API_REF_CLASS_DATA_MEMBER(type, member)

#define REF_CLASS_END(type)                 DRV_API_REF_CLASS_END(type)
#else
...
#define REF_CLASS_DATA_MEMBER(type, member)               \
  decltype(std::declval<type>().member)& member() const { \
    return ptr_->member;                                  \
  }
