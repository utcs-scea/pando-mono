...
static int& column(std::pair<int, float>& pair) {
  return pair.first;
}
 ...
 */
static void transpose_nonzero(int& row, int& nz) {
  int tmp = row;
  row = nz;
  nz = tmp;
}
 ...
 */
static void transpose_nonzero(int& row, std::pair<int, float>& pair) {
  int tmp = row;
  row = pair.first;
  pair.first = tmp;
}
 ...
 */
static int read_scan_nonzero(const std::string& fname, FILE* f, const MM_typecode& banner, int& row,
                             int& nz) {
  int s, d, vi;
  float vd;
  int r = 0;
 ...
 */
static void read_sort_row(std::vector<int>& v) {
...
static int read_common(const std::string& graph,
                       /* outputs */
                       int* V, int* E, std::vector<int>& offsets, std::vector<NonZeroT>& nonzeros) {
  // 1. open the file
...
int read_sparse_matrix(const std::string& sparse_matrix,
                       /* output */
