 ...
 */
DrvAPIPAddress DrvAPIVAddress::to_physical(uint32_t this_pxn, uint32_t this_pod,
...
DrvAPIPAddress DrvAPIVAddress::to_physical(DrvAPIAddress addr, uint32_t this_pxn, uint32_t this_pod,
                                           uint32_t this_core_y, uint32_t this_core_x) {
...
std::string DrvAPIVAddress::to_string() {
  ...
  if (is_ctrl_register()) {
    ...
    type << "L1SP";
    if (global()) {
      locale << "PXN=" << pxn() << " POD=" << pod() << " CORE_Y=" << core_y()
             << " CORE_X=" << core_x();
    } else {
      locale << "LOCAL";
    }
