...
atomic_modify(IntType w, IntType r, DrvAPIMemAtomicType op) {
    switch (op) {
...
template <typename IntType>
std::pair<IntType,IntType>
atomic_modify(IntType w, IntType r, IntType ext, DrvAPIMemAtomicType op) {
    switch (op) {
    case DrvAPIMemAtomicCAS:
        if (r == ext) {
            return {w, r};
...
inline void atomic_modify(void *w, void *r, void *ext, void *o, DrvAPIMemAtomicType op, std::size_t sz) {
    switch (sz) {
    case 1:
        atomic_modify
            (static_cast<std::uint8_t *>(w)
            ,static_cast<std::uint8_t *>(r)
            ,static_cast<std::uint8_t *>(ext)
            ,static_cast<std::uint8_t *>(o)
            ,op
            );
        break;
