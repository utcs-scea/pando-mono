...
T atomic_swap(DrvAPIAddress address, T value) {
  ...
  DrvAPIThread::current()->setState(
      std::make_shared<DrvAPIMemAtomicConcrete<T, DrvAPIMemAtomicSWAP>>(address, value));
  DrvAPIThread::current()->yield();
...
T atomic_add(DrvAPIAddress address, T value) {
  T result = 0;
  DrvAPIThread::current()->setState(
      std::make_shared<DrvAPIMemAtomicConcrete<T, DrvAPIMemAtomicADD>>(address, value));
  DrvAPIThread::current()->yield();
  auto atomic_req = std::dynamic_pointer_cast<DrvAPIMemAtomic>(DrvAPIThread::current()->getState());
  if (atomic_req) {
    atomic_req->getResult(&result);
...
T atomic_or(DrvAPIAddress address, T value) {
  T result = 0;
  DrvAPIThread::current()->setState(
      std::make_shared<DrvAPIMemAtomicConcrete<T, DrvAPIMemAtomicOR>>(address, value));
  DrvAPIThread::current()->yield();
  auto atomic_req = std::dynamic_pointer_cast<DrvAPIMemAtomic>(DrvAPIThread::current()->getState());
  if (atomic_req) {
    atomic_req->getResult(&result);
...
T atomic_cas(DrvAPIAddress address, T compare, T value) {
  T result = 0;
  DrvAPIThread::current()->setState(
      std::make_shared<DrvAPIMemAtomicConcreteExt<T, DrvAPIMemAtomicCAS>>(address, value, compare));
  DrvAPIThread::current()->yield();
  auto atomic_req = std::dynamic_pointer_cast<DrvAPIMemAtomic>(DrvAPIThread::current()->getState());
  if (atomic_req) {
    atomic_req->getResult(&result);
