...
#define DRV_API_REF_CLASS_DATA_MEMBER(type, member)                                               \
  DrvAPI::DrvAPIPointer<decltype(std::declval<type>().member)>::value_handle member() const {     \
    return *DrvAPI::DrvAPIPointer<decltype(std::declval<type>().member)>(ptr_.vaddr_ +            \
