...
public:
   ...
   */
  virtual uint64_t getBase(uint32_t pxn, uint32_t pod, uint32_t core) const = 0;

  /**
   * @brief set the section base
   */
  virtual void setBase(uint64_t base, uint32_t pxn, uint32_t pod, uint32_t core) = 0;
...
public:
  ...
  DrvAPIPointer<T> pointer() const {
    DrvAPIAddress base =
        DrvAPISection::GetSection(MEMTYPE).getBase(myPXNId(), myPodId(), myCoreId());
   ...
   * address operator
   */
  DrvAPIPointer<T> operator&() const {
    return pointer();
  }
...
public:
   ...
   * cast operator
   */
  operator DrvAPIPointer<T>() const {
    return static_cast<DrvAPIPointer<T>>(*pointer());
  }
   ...
   * address operator
   */
  DrvAPIPointer<DrvAPIPointer<T>> operator&() const {
    return pointer();
  }
   ...
   * subscript operator
   */
  typename DrvAPIPointer<T>::value_handle operator[](size_t idx) const {
