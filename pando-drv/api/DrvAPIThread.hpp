...
public:
   ...
   * @param state
   */
  void setState(const std::shared_ptr<DrvAPIThreadState>& state) {
    state_ = state;
  }
   ...
   */
  std::shared_ptr<DrvAPIThreadState>& getState() {
    return state_;
  }
   ...
   * @param main
   */
  void setMain(drv_api_main_t main) {
    main_ = main;
  }
  ...
  void setCoreThreads(int threads) {
    core_threads_ = threads;
  } //!< Set the number of threads on this core
  ...
  int podId() const {
    return pod_id_;
  } //!< Get the pod id w.r.t pxn

  /**
   * @brief set the pod id w.r.t pxn
   */
  void setPodId(int pod_id) {
    pod_id_ = pod_id;
  } //!< Set the pod id w.r.t pxn
...
private:
  ...
  std::unique_ptr<coro_t::pull_type>
      thread_context_;                       //!< Thread context, coroutine that can be resumed
