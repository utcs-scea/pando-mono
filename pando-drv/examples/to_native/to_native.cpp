...
#define pr_info(fmt, ...)                                                                   \
  do {                                                                                      \
    printf("INFO:  PXN %3d: POD: %3d: CORE %3d: " fmt "", myPXNId(), myPodId(), myCoreId(), \
           ##__VA_ARGS__);                                                                  \
  } while (0)
#else
#define pr_info(fmt, ...) \
...
#define pr_error(fmt, ...)                                                                  \
  do {                                                                                      \
    printf("ERROR: PXN %3d: POD: %3d: CORE %3d: " fmt "", myPXNId(), myPodId(), myCoreId(), \
           ##__VA_ARGS__);                                                                  \
  } while (0)
...
int ToNativeMain(int argc, char* argv[]) {
  ...
  for (auto simaddr : test_addresses) {
    ...
    uint64_t wvalue =
        addr.to_physical(myPXNId(), myPodId(), myCoreId() >> 3, myCoreId() & 0x7).encode();
