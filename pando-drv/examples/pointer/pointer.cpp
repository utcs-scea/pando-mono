...
#define pr(fmt, ...)                                                           \
  do {                                                                         \
    printf("Core %4d: Thread %4d: " fmt "", DrvAPIThread::current()->coreId(), \
           DrvAPIThread::current()->threadId(), ##__VA_ARGS__);                \
  } while (0)
...
public:
  ...
  DrvAPI::DrvAPIPointer<decltype(std::declval<foo>().baz)>::value_handle baz() {
    return *DrvAPI::DrvAPIPointer<int>(_fooptr.vaddr_ + offsetof(foo, baz));
  }

  DrvAPI::DrvAPIPointer<decltype(std::declval<foo>().bar)>::value_handle bar() {
    return *DrvAPI::DrvAPIPointer<float>(_fooptr.vaddr_ + offsetof(foo, bar));
  }
...
int PointerMain(int argc, char* argv[]) {
  ...
  if (DrvAPIThread::current()->threadId() == 0 && DrvAPIThread::current()->coreId() == 0) {
    pr("%s\n", __PRETTY_FUNCTION__);
