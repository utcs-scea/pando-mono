...
DRV_API_REF_CLASS_BEGIN(frontier_data)
  ...
  void print_async() {
    std::cout << "Thread " << my_thread() << ": printing frontier "
              ...
              << "size = " << size() << ", "
              << "is_dense = " << is_dense() << ", "
              << "vertices = " << std::hex << std::setw(8) << std::setfill('0') << static_cast<Pointer<int>>(vertices()) << ", "
              << std::dec << std::endl;
  }
...
int BFSMain(int argc, char *argv[])
{
    ...
    if (my_thread() == 0) {
        ...
        if (graph_file == "") {
            // std::cout << "Usage: " << std::string(argv[0]) << " <graph_file> [root_vertex (default 0)]" << std::endl;
            barrier.sync();
            return 1;
        }
    ...
    //           << ": curr_frontier = " << std::hex << static_cast<uint64_t>(&curr_frontier) << std::dec << std::endl;
    // std::cout << "Thread " << my_thread()
    //           << ": next_frontier = " << std::hex << static_cast<uint64_t>(&next_frontier) << std::dec << std::endl;
    int iter = 0;
    while (curr_frontier.size() != 0) {
        int d = iter + 1;
        barrier.sync([=](){
            std::cout << "Iteration " << std::setw(2) << iter
                      << ": " << std::setw(3) << curr_frontier.size()
    ...
    if (my_thread() == 0) {
        for (int v = 0; v < V; v++) {
            if (l_distance[v] != distance[v]) {
                std::cout << "ERROR: distance[" << v << "] = " << l_distance[v] << ", expected " << distance[v] << std::endl;
            }
