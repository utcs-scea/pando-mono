...
struct CSRInterface {
  ...
  bool localVertexPos(size_t n) {
    return n < VArrSz / SIM_PXN;
  }
  bool localEdgePos(size_t n) {
    return n < EArrSz / SIM_PXN;
  }
...
struct IDMVCache {
  ...
  IDMVCache(DrvAPIPointer<Vertex> value, DrvAPIPointer<uint32_t> arg1, DrvAPIPointer<bool> lock,
            size_t size)
...
struct IDMSamplingCache {
  ...
  IDMSamplingCache(DrvAPIPointer<Edge> value, DrvAPIPointer<uint32_t> arg1,
                   DrvAPIPointer<uint8_t> arg2, DrvAPIPointer<bool> lock, size_t size)
...
void ComputeThread(CSRInterface& csr, IDMCacheA& idmVCache, IDMCacheB& idmECache) {
  ...
  if ((int)tid == totalComputeThreads() - 1)
    end = totalRoot;
  ...
  DrvAPIPointer<Edge> neighborhood = DrvAPIMemoryAlloc(DrvAPIMemoryDRAM, sizeof(Edge) * 5);
    ...
    while (frontier_head < frontier_tail) {
      ...
      if (edges_to_fetch) {
        ...
        if (idmECache.lookup(glbID, edges_to_fetch, neighborhood)) {
          ...
          if (!csr.localEdgePos(startEL))
            hitRemoteECnt += edges_to_fetch;
          else
            hitLocalECnt += edges_to_fetch;
        } else {
  ...
  printf(
      "E local accesses: %lu (from IDM cache: %lu [%.2lf%%])\nE remote accesses: %lu (from IDM "
      "cache: %lu [%.2lf%%])\n",
      localECnt, hitLocalECnt, ((double)hitLocalECnt / localECnt * 100.0), remoteECnt,
      hitRemoteECnt, ((double)hitRemoteECnt / remoteECnt * 100.0));
...
void IDMThread(CSRInterface& csr, IDMCacheA& idmVCache, IDMCacheB& idmECache) {
  {
    while (g_threadStatus[myPairID()] == 0)
      DrvAPI::wait(IDM_BARRIER_CYCLES);
  }
...
int AppMain(int argc, char* argv[]) {
  ...
  auto idmSCacheValue =
      static_cast<DrvAPIPointer<Edge>>(g_idmSCacheValue) + cacheOffset * IDMCacheB::MS;
