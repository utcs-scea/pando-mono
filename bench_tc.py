...
rmat_files = {
    3:"graphs/kron_default_3_out.el",
    4:"graphs/kron_default_4_out.el"
...
def collect_traces_RMAT(num_hosts, graph_mode, chunk_mode, scale, machine, output_dir='data', exec2run='tc', build='dockerbuild'):
    if build == 'dockerbuild-drv':
        output_dir += "/DRV"
    ...
    if build == 'dockerbuild' and not os.path.exists(fn):
        print(f"\t *** Creating {fn}")
        os.makedirs(fn)
        assert os.path.exists(fn), f"OS should now see a path to {fn}"

    executable = f"./{build}/microbench/triangle-counting/src/{exec2run}"
    cmd0 = ""
    if build=='dockerbuild-drv':
        cmd0 = f"./scripts/run-drv.sh -n {num_hosts} -c {num_cores} ./dockerbuild-drv/microbench/triangle-counting/src/libtc.so "
    else:
        ...
        cmd0 += f"./scripts/preprun_mpi.sh -c {num_cores} -n {num_hosts} {executable} "

    cmd0 += f"-i {input_graph} -v {num_input_vertices} -c {chunk_mode} -g {graph_mode}"
...
def experiment_chunk_DLCSR_RMAT(num_hosts, machine, output_dir='data', build='dockerbuild'):
    print("*******************************************************")
    ...
    for scale in rmat_files:
        ...
        for c in range(len(chunk_modes)):
            cmds = collect_traces_RMAT(num_hosts, 0, c, scale, machine, output_dir=f'{output_dir}/expChunk', build=build)
            all_cmds += cmds

    with open(f"experiment_{build}_chunk_{machine}_hosts{num_hosts}.sh", "w") as file:
        for line in all_cmds:
            file.write(f"{line}\n")

def experiment_graph_type_RMAT(num_hosts, machine, output_dir='data',build='dockerbuild'):
    print("*******************************************************")
    ...
    for scale in rmat_files:
        ...
        for g in range(len(graph_types)):
            input_graph = rmat_files[scale]
            cmds = collect_traces_RMAT(num_hosts, g, 0, scale, machine, output_dir=f'{output_dir}/expGT', build=build)
            all_cmds += cmds

    with open(f"experiment_{build}_graph_type_{machine}_hosts{num_hosts}.sh", "w") as file:
        for line in all_cmds:
            file.write(f"{line}\n")


def experiment_graph_construction_RMAT(num_hosts, machine, output_dir='data', exec2run='graph_compare',build='dockerbuild'):
    print("*******************************************************")
    print(f"          Collecting Benchmarks for GRAPH TYPES ...")
    print("*******************************************************")
    all_cmds = []
    for scale in rmat_files:
        ...
        for g in range(len(graph_types)):
            input_graph = rmat_files[scale]
            cmds = collect_traces_RMAT(num_hosts, g, 0, scale, machine, output_dir=f'{output_dir}/exp_graph_construc', exec2run=exec2run, build=build)
            all_cmds += cmds

    with open(f"experiment_{build}_graph_construc_{machine}_hosts{num_hosts}.sh", "w") as file:
        for line in all_cmds:
            file.write(f"{line}\n")
...
parser.add_argument(
    ...
    "--build",
    action="store",
    help=f"Enter build (default = dockerbuild)",
    type=str,
    required=False,
)
parser.add_argument(
...
experiment_chunk_DLCSR_RMAT(num_hosts=num_hosts, output_dir=output_dir, machine=machine,build=build)
experiment_graph_type_RMAT(num_hosts=num_hosts, output_dir=output_dir, machine=machine,build=build)
experiment_graph_construction_RMAT(num_hosts=num_hosts, output_dir=output_dir, machine=machine,build=build)
